...
BASE_DIR="${BASE_DIR:-/data/bwh-comppath-full/gerberlab/jy1008/SaMu/metagenomics}"

RAW_DIR="$BASE_DIR/data/raw"                # combined + QC'd reads
MAPPING_DIR="$BASE_DIR/data/mapping"        # bowtie2 host-mapping intermediates
HOST_REMOVED_DIR="$BASE_DIR/data/host_removed"
KRAKEN_DIR="$BASE_DIR/results/kraken_out"
METAPHLAN_DIR="$BASE_DIR/results/metaphlan_out"
...
SLURM_PARTITION="${SLURM_PARTITION:-bigmem}"
...
# ---------------------------------------------------------------------------
# MetaPhlAn (v31, pre-SGB) — standalone run whose output feeds HUMAnN via
# --taxonomic-profile. HUMAnN 3.x's ChocoPhlAn (v201901_v31) only matches
# pre-SGB species names, so this uses the mpa_v31_CHOCOPhlAn_201901 database
# (MetaPhlAn 3.1.0), NOT the vJun23/vOct22 SGB-based database used elsewhere.
# ---------------------------------------------------------------------------
METAPHLAN_V31_ENV="${METAPHLAN_V31_ENV:-metaphlan31_env}"
METAPHLAN_V31_BOWTIE2_DB="${METAPHLAN_V31_BOWTIE2_DB:-/data/bwh-comppath-full/databases/metaphlan_v31}"
METAPHLAN_V31_INDEX="${METAPHLAN_V31_INDEX:-mpa_v31_CHOCOPhlAn_201901}"
METAPHLAN_V31_DIR="$BASE_DIR/metaphlan_v31_out_nocounts"

SLURM_METAPHLAN_V31_TIME="${SLURM_METAPHLAN_V31_TIME:-8:00:00}"
SLURM_METAPHLAN_V31_MEM="${SLURM_METAPHLAN_V31_MEM:-32G}"

# ---------------------------------------------------------------------------
# HUMAnN — consumes the v31 MetaPhlAn profile above via --taxonomic-profile.
# No internal MetaPhlAn invocation; ChocoPhlAn is v201901_v31 (pre-SGB),
# matching the v31 profile's species naming.
# ---------------------------------------------------------------------------
HUMANN_DIR="$BASE_DIR/humann_out"

HUMANN_CHOCOPHLAN_DB="${HUMANN_CHOCOPHLAN_DB:-/data/bwh-comppath-full/databases/humann/chocophlan/chocophlan}"
HUMANN_UNIREF_DB="${HUMANN_UNIREF_DB:-/data/bwh-comppath-full/databases/humann/uniref/uniref}"

SLURM_HUMANN_TIME="${SLURM_HUMANN_TIME:-18:00:00}"
SLURM_HUMANN_MEM="${SLURM_HUMANN_MEM:-48G}"

HUMANN_ENV="${HUMANN_ENV:-humann39_env}"
